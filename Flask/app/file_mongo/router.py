...
import app.file_mongo.model as file_model
import app.file_mongo.factory as factory
import app.file_mongo.validator as validator
import app.ingredient.model as ingredient_model
import app.recipe.model as recipe_model

apis = Blueprint('file_mongo', __name__, url_prefix='/file_mongo')

server = utils.Server()
file = file_model.FileMongo()
ingredient = ingredient_model.Ingredient()
recipe = recipe_model.Recipe()


@apis.route('/<_id>', methods=['DELETE'])
def delete_file_mongo(_id):
    """
    @api {delete} /file/<_id_file>  DeleteFileMongo
    @apiGroup FileMongo
...
def get_file_mongo(_id):
    """
    @api {get} /file/<_id_file>  DownloadFileMongo
    @apiGroup FileMongo
...
def post_ingredient_file_mongo(_id):
    """
    @api {post} /file/ingredient/<_id_ingredient>  PostIngredientFileMongo
    @apiGroup FileMongo
...
def post_recipe_file_mongo(_id):
    """
    @api {post} /file/recipe/<_id_recipe>  PostRecipeFileMongo
    @apiGroup FileMongo
...
def post_step_file_mongo(_id_recipe, _id_step):
    """
    @api {post} /file/recipe/<_id_recipe>/step/<_id_step> PostStepFileMongo
    @apiGroup FileMongo
...
def put_file_is_main_mongo(_id):
    """
    @api {put} /file/<_id_file>  PutFileIsMainMongo
    @apiGroup FileMongo
