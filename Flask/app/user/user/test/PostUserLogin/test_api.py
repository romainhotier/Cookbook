...
import app.user.user.test.PostUserLogin.api as api
...
class PostUserLogin(unittest.TestCase):
    ...
    def test_0_api_ok(self):
        tc_user = user_model.UserTest().custom_test({}).insert()
        body = {api.param_email: tc_user.get_data_value("email"),
                api.param_password: tc_user.get_data_value("password")}
        """ call api """
    ...
    def test_2_email_without(self):
        tc_user = user_model.UserTest().custom_test({}).insert()
        body = {api.param_password: tc_user.get_data_value("password")}
        """ call api """
        url = server.main_url + "/" + api.url
        response = requests.post(url, json=body, verify=False)
        response_body = response.json()
