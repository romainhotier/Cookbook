...
import utils
import tests.recipe.PutRecipeSteps.api as api
import tests.recipe.model as recipe_model
import tests.file.model as file_model

server = utils.Server()
api = api.PutRecipeSteps()
recipe = recipe_model.RecipeTest()
file = file_model.FileTest()


class PutRecipeSteps(unittest.TestCase):
    ...
    def test_0_api_ok_without_position(self):
        ...
        tc_recipe = recipe_model.RecipeTest().custom({"steps": [{"_id": ObjectId("111111111111111111111111"),
                                                                 "description": "step_ini"}]}).insert()
        """ param """
        tc_id = tc_recipe.get_id()
        body = {api.param_steps: [{api.param_description: "new_step1"},
                                  {api.param_description: "new_step2"}]}
        """ call api """
