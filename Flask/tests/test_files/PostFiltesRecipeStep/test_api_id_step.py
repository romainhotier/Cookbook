...
class TestPostFilesRecipeStep(unittest.TestCase):
    ...
    def test_id_step_empty(self):
        """ QueryParameter _id_step is an empty string.
    ...
    def test_id_step_invalid(self):
        ...
        self.assertTrue(rep.check_not_present(value="data", response=response_body))
        detail = rep.format_detail(param=api.param_id_step, msg=rep.detail_must_be_an_object_id, value=tc_id_step)
    ...
    def test_id_step_object_id_invalid(self):
        ...
        self.assertEqual(response_body["codeMsg"], api.rep_code_msg_error_400)
        self.assertTrue(rep.check_not_present(value="data", response=response_body))
        detail = rep.format_detail(param=api.param_id_step, msg=rep.detail_doesnot_exist, value=tc_id_step)
        self.assertEqual(response_body["detail"], detail)
        """ check recipe """
        tc_recipe.check_bdd_data()
        """ check files """
