...
class PostIngredientFile(unittest.TestCase):
    ...
    def test_is_main_without(self):
        ...
        tc_file = tc_ingredient.add_file_mongo(filename=body[api.param_filename], is_main=False)
        """ assert """
    ...
    def test_is_main_true_already_exist(self):
        ...
        """ env """
        tc_ingredient = ingredient_model.IngredientTest().insert()
        tc_file1 = tc_ingredient.add_file_mongo(filename="qa_rhr_filename", is_main=True)
        """ param """
        tc_id = tc_ingredient.get_id()
        body = {api.param_path: file_path,
        ...
        new_id = api.return_new_file_id(response_body)
        tc_file2 = tc_ingredient.add_file_mongo(filename=body[api.param_filename], is_main=body[api.param_is_main])
        """ assert """
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.headers["Content-Type"], "application/json")
        self.assertEqual(response_body["codeStatus"], 201)
