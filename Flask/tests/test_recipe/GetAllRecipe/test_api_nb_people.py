...
class TestGetAllRecipe(unittest.TestCase):
    ...
    def test_nb_people_empty(self):
        ...
        detail = rep.format_detail(param=api.param_nb_people, msg=rep.detail_must_be_an_integer, value=tc_nb_people)
    ...
    def test_nb_people_ok(self):
        ...
        """
        """ env """
        tc_recipe1 = RecipeTest().custom({"nb_people": 5}).insert()
        tc_recipe2 = RecipeTest().custom({"nb_people": 6}).insert()
        """ param """
        tc_nb_people = "5"
        """ call api """
        url = server.main_url + "/" + api.url + "?" + api.param_nb_people + "=" + tc_nb_people
